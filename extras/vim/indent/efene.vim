...
setlocal indentkeys+=0=end,0=:,0=try,0=match,0=catch,0=after,0=when,0=),0=],0=}
...
function! s:IsCatchStandalone(lnum, i)
  ...
  elseif prev_token =~# '[a-z]'
    if index(['after', 'and', 'andd', 'begin', 'not',
            \ 'case', 'catch', 'or', 'orr',
            \ 'try', 'xor'], prev_token) != -1
...
function! s:ErlangCalcIndent2(lnum, stack)
  ...
  while 1
    ...
    while i >= 0
      ...
      elseif (index([':', 'receive', 'after', 'when'], token) != -1 ||
      ...
      elseif index(['case', 'when', 'try', 'receive'], token) != -1
