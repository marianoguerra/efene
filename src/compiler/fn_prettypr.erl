...
%% 					 floating(text(")"))))),
	...
	if_expr ->
	    ...
	    D = lay_clauses(erl_syntax:if_expr_clauses(Node),
			    if_expr, Ctxt1),
	    sep([par([follow(text("if"), D, Ctxt1#ctxt.sub_indent)], Ctxt1#ctxt.break_indent)]);
	...
	fun_expr ->
	    ...
	    D = lay_clauses(erl_syntax:fun_expr_clauses(Node),
			       fun_expr, Ctxt1),
	    sep([follow(text("fn"), D, Ctxt1#ctxt.sub_indent),
                text("}")]);
...
lay_clauses(Cs, Type, Ctxt) ->
    vertical(seq(Cs, floating(text("}")), % XXX this is the end of a clause that has more definitions (; in erlang)
...
append_clause_body(B, D, Ctxt) ->
    append_clause_body(B, D, floating(text(" {")), Ctxt). % XXX this is the -> in erlang
